<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold text-slate-900 mb-6">Edit Jadwal</h1>
        ...
        <form action="{{ route('dosen.jadwal.update', $jadwal->id) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="hari" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach($hari as $h)
                        <option value="{{ $h }}" @if($jadwal->hari === $h) selected @endif>{{ $h }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="jam_mulai" required value="{{ $jadwal->jam_mulai }}"
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900 mb-2">Jam Selesai</label>
                    <input type="time" name="jam_selesai" required value="{{ $jadwal->jam_selesai }}"
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="kegiatan" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach($kegiatan as $k)
                        <option value="{{ $k }}" @if($jadwal->kegiatan === $k) selected @endif>{{ $k }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="ruangan" placeholder="Contoh: Lab WICIDA, Ruang A.1" value="{{ $jadwal->ruangan }}"
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" placeholder="Tambahkan catatan atau keterangan tambahan..."
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $jadwal->keterangan }}</textarea>
            </div>
